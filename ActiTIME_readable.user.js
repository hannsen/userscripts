...
// @author       You
// @match        https://actitime.quodata.de/actitime/user/submit_tt.do*
// @downloadURL  https://git04.quodata.de/quodata/userscripts/-/raw/master/actiTIME_readable.user.js
// @updateURL    https://git04.quodata.de/quodata/userscripts/-/raw/master/actiTIME_readable.user.js
// @grant        none
// ==/UserScript==
...
(function() {
    ...
    $('div.task').css('font-size', '14px');
    $('div.task').css('color', 'black');
    $('span.customer').css('color', '#403f3e');
    $('span.project').css('color', '#403f3e');
    ...
    $('tr.taskRow').each(function(){
       var customer = $(this).find('span.customer').text();
       var hash = md5(customer);
       var color = '#' + hash.substr(hash.length - 6);
       color = shadeColor(color, 180);
    ...
    $('td.leaveButtonCell:has(table.nonworking)').each(function(){
       var index = $(this).closest('td')[0].cellIndex;
        ...
        $('#actualTTRows tr.taskRow td:nth-of-type('+ index +') input.inputTT').prop( "disabled", true ).fadeTo(0, 0.5);
    });
})();
